...
interface SecurityTabWorld {
  ...
  runId: string;
  testOrg: string;
  skills: Map<string, { verdict: string | null; findings: number; hasReadme: boolean; hasDescription: boolean; hasLicense: boolean; hasRepo: boolean; hasPermissions: boolean }>;
  lastStatus: number;
  lastBody: string;
  lastHtml: string;
}
...
async function seedSkillWithOptions(
  ...
  const { verdict, critical = 0, high = 0, medium = 0, low = 0, hasReadme = false, hasDescription = false, hasLicense = false, hasRepo = false, hasPermissions = false } = options;
  ...
  const permissions: Record<string, unknown> = hasPermissions
    ? { network: { outbound: ["https://api.example.com"] }, filesystem: { read: ["/tmp"], write: [] }, subprocess: false }
    : {};
