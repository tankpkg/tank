...
function givenNoDirectoryExists(): string {
  const missingDir = path.join(
    os.tmpdir(),
    `tank-bdd-nonexistent-${Date.now()}-${Math.random().toString(16).slice(2)}`,
  );
  ...
  describe("Scenario: Agent initialises a new skill project with all required fields", () => {
    it("Given/When/Then for successful project initialization", async () => {
      ...
      try {
        ...
        thenResponseContains(/tank\.json/i);
  ...
  describe("Scenario: Agent initialises a skill project when tank.json already exists", () => {
    it("Given/When/Then for existing tank.json without overwrite", async () => {
      ...
      try {
        ...
        thenToolCompletesWithoutError();
        thenResponseContains(/tank\.json already exists/i);
