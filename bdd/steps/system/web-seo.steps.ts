...
function parseHomepageOgImageUrl(source: string): string {
  ...
  const url = match?.[1];
  if (!url) {
    throw new Error('Expected homepage openGraph image URL in layout metadata');
  }
  return url;
}
...
function parseSkillOgImageUrl(source: string): string {
  ...
  const url = match?.[1];
  if (!url) {
    throw new Error('Expected dynamic skill ogImageUrl in skill detail metadata');
  }
  return url;
}
...
function parseImageDimensionsFromSource(source: string): ImageDimensions {
  ...
  if (!width || !height) {
    throw new Error('Expected image dimensions in source');
  }
  return {
    width: Number(width),
...
function thenResponseContainsMetaTagWithProperty(property: string): void {
  ...
  const openGraphContent = openGraphBlock?.[1];
  if (!openGraphContent) {
    throw new Error('Expected openGraph metadata block');
  }
  expect(openGraphContent).toContain(`${field}:`);
}
...
function thenSkillDetailContainsMetaTagWithProperty(property: string): void {
  ...
  const openGraphContent = openGraphBlock?.[1];
  if (!openGraphContent) {
    throw new Error('Expected openGraph metadata block in skill detail page');
  }
  expect(openGraphContent).toContain('images:');
...
function thenResponseContainsTwitterMetaTag(name: string, content?: string): void {
  ...
  const twitterContent = twitterBlock?.[1];
  if (!twitterContent) {
    throw new Error('Expected twitter metadata block');
  }

  if (name === 'twitter:card') {
    expect(twitterContent).toContain('card:');
    if (content) expect(twitterContent).toContain(`'${content}'`);
    return;
  }

  if (name === 'twitter:image') {
    expect(twitterContent).toContain('images:');
  }
}
