...
- [x] **1.2** Create `apps/registry/lib/branding.ts` with `getBrandConfig()` function
- [x] **1.3** Add all `BRAND_*` env vars to `apps/registry/.env.example`
- [x] **1.4** Add CSS custom properties `--brand-*` in `apps/registry/app/globals.css`
...
- [x] **2.1** Create `Logo` component (`apps/registry/components/logo.tsx`)
