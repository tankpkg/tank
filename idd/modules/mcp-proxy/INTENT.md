...
- `joergmichno/clawguard` (MIT) — 55 exported regex patterns across 7
  categories (pinned ref `4ec3e09`) + leet-speak and whitespace normalization.
  Upstream documentation sometimes cites "216" — that count reflects internal
  pattern variants, not the TypeScript-importable exported list. See
  `decisions.md` D13.
...
packages/internals-helpers/src/
  ...
  prompt-injection/
    patterns.ts               # ClawGuard 55-pattern set (7 categories) for prompt injection / hidden instructions (consumed by tool-description scanner C8 and resources/prompts scanner C32-C34). Generated from vendor/clawguard submodule by scripts/codegen-clawguard.ts; committed to git.
...
| C8  | Detection uses ClawGuard-derived 55 regex patterns (7 categories) with evasion normalization                                                                                                                                                                                                                                                        | Covers zero-width chars, homoglyphs, base64-encoded instructions, leet-speak                                                                | BDD scenario |
| C9  | Normalization pipeline (5 stages, fixed order per D15): (1) strip zero-width codepoints → (2) NFKC + homoglyph decode → (3) detect + decode base64 substrings → (4) ClawGuard leet-speak reversal → (5) whitespace collapse — then scan. Base64-decode recursion is bounded at depth 3 and total decoded bytes ≤ 64 KB to preserve C10 perf budget. | Evasion techniques must be neutralized before pattern matching; disjoint evasion classes require both Tank (1–3) and ClawGuard (4–5) stages | Unit test    |
...
| C14  | Pinned hashes stored in `~/.tank/proxy/pins/<package-hash>.json` (one file per package); directory is auto-created with mode `0700` on first write                                                                                                                                                                                                 | Persists across proxy restarts; isolates failures (one corrupt pin ≠ all pins lost); `0700` keeps pins private to the installing user                                                                                                                                                                                 | Unit test    |
| C14a | `<package-hash>` in Phase 2 is SHA-256 hex of the canonicalized proxy argv: `JSON.stringify([resolvedExe, ...resolvedArgs])` after whitespace-trim per element. For `tank proxy --remote <url>`, the canonical form is `['--remote', <url>]`. Phase 6 adapter rewriting will upgrade to a package-manifest-derived hash and migrate existing pins. | Gives Phase 2 a deterministic identity key without requiring adapter rewriting (which ships in Phase 6). argv-based identity is stable across restarts of the same invocation and distinct across semantically-different invocations. Migration in Phase 6 is a one-time rename of files under `~/.tank/proxy/pins/`. | Unit test    |
| C14b | Pin writes use atomic rename with a per-writer unique temp file: `<hash>.json.tmp.<pid>.<random8>` (where `<random8>` = 4 bytes from `crypto.randomBytes` rendered as hex) → `fs.rename()` onto `<hash>.json`. On proxy startup, a sweep deletes any `<hash>.json.tmp.*` older than 1 hour. No advisory lockfile.                                  | Per-writer temp filename eliminates the concurrent-writer race where two processes would stomp a shared `<hash>.json.tmp`. Atomic rename on the same filesystem is POSIX-atomic. Startup sweep reclaims temp files from crashed writers without requiring lock-recovery logic.                                        | Unit test    |
| C14c | Pin file read failures (`ENOENT` is not a failure — treated as "first run"; any other error including corrupt JSON, `EACCES`, `EIO`) **fail closed**: the `tools/list` response is rejected with JSON-RPC error `-32002` (`"tank: pin read failed"`) and the tools are NOT returned to the agent. The proxy does not silently re-pin.              | Prevents the attack where an adversary corrupts or makes unreadable the pin file to force silent re-pinning and bypass rug-pull detection. `ENOENT` stays benign so first-run flow still works.                                                                                                                       | BDD scenario |
| C15  | `tank proxy --reset-pins` clears stored hashes to re-approve; deletes every file directly under `~/.tank/proxy/pins/` (non-recursive; directory itself preserved). Prints count deleted. Does not exit the proxy.                                                                                                                                  | Escape hatch for legitimate updates; usable in CI/script mode before launching                                                                                                                                                                                                                                        | BDD scenario |
...
| C34 | Resources/prompts scanning applies the same C9 normalization pipeline and the same ClawGuard 55-pattern prompt-injection set (C8) as tool descriptions. Credential detection (C23/C25/C25a) runs in parallel using its own pattern set + entropy gate. | Single prompt-injection detection pipeline; credential detection is a separate, orthogonal concern | Unit test    |
