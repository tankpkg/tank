...
- use `@internals/schemas` for shared schemas and contract constants
- use `@internals/helpers` for shared pure helpers
...
- keep `@internals/schemas` and `@internals/helpers` pure
- export public API through each package `src/index.ts`
