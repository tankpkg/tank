...
def validate_download_url(url: str) -> None:
    ...
    is_allowed = any(hostname == allowed or hostname.endswith(f".{allowed}") for allowed in ALLOWED_DOWNLOAD_DOMAINS)

    if not is_allowed:
        raise ValueError(
            f"URL must be from authorized storage domain. Got: {hostname}, Allowed: {ALLOWED_DOWNLOAD_DOMAINS}"
        )
