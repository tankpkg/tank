...
def check_skill_md_exists(temp_dir: str) -> Finding | None:
...
def check_nfkc_normalization(content: str, file_path: str) -> Finding | None:
...
def check_encoding(file_path: str) -> Finding | None:
        ...
        try:
            ...
            findings.append(
                Finding(
                    ...
                    description=f"Could not read file: {e!s}",
                    location=file_path,
                    confidence=1.0,
                    tool="stage1_structure",
                )
