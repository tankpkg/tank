...
class PermissionsRequest(BaseModel):
    skill_dir: str | None = None  # Path to extracted skill directory
    skill_content: str | None = None  # Fallback for compatibility
        ...
        try:
            ...
            if not is_within_base or not requested_path.is_dir():
                return JSONResponse(
                    status_code=400,
                    content={
                        "error": "Invalid skill_dir: must refer to an existing directory within the configured skills base directory."
                    },
                )
