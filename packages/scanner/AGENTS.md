...
```bash
cd apps/scanner && pytest                 # All Python tests
...
**CRITICAL**: Any changes to `apps/scanner/` must be mirrored to `apps/web/api-python/`:

```bash
cp -r apps/scanner/lib/scan/* apps/web/api-python/analyze/scan/
cp -r apps/scanner/api/analyze/* apps/web/api-python/analyze/
```
