      ...
      <div className="space-y-4">
        <form method="get" className="flex flex-wrap gap-3 items-end">
          <div className="w-full sm:w-64">
            ...
            <Input id="search" name="search" placeholder="Organization name or slug" defaultValue={search} />
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            {currentPage > 1 ? (
              <Button variant="outline" asChild>
                <Link href={`/admin/orgs?${buildQuery(searchParams, currentPage - 1)}`}>Previous</Link>
              </Button>
            ) : (
            ...
            {currentPage < totalPages ? (
              <Button asChild>
                <Link href={`/admin/orgs?${buildQuery(searchParams, currentPage + 1)}`}>Next</Link>
              </Button>
            ) : (
...
export default async function AdminOrgsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  const resolvedSearchParams = await searchParams;
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Organizations</h1>
        <p className="text-muted-foreground mt-1">View and manage all organizations in the registry.</p>
      </div>
