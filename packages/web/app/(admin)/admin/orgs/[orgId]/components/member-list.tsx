...
export function MemberList({ orgId, members }: { orgId: string; members: OrgMember[] }) {
  ...
  return (
    <div className="space-y-3">
      {members.map((member) => (
        <div key={member.id} className="rounded-md border p-3 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" size="sm" onClick={() => onRemove(member.id)} disabled={isPending}>
              Remove
            </Button>
          </div>
        </div>
