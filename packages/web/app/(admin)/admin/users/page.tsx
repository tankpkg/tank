      ...
      <div className="space-y-4">
        <form method="get" className="flex flex-wrap gap-3 items-end">
          <div className="w-full sm:w-64">
            ...
            <Input id="search" name="search" placeholder="Name or email" defaultValue={search} />
          </div>
                ...
                data.users.map((user) => {
                  ...
                  return (
                    <TableRow key={user.id} className="hover:bg-muted/40">
                      ...
                      <TableCell>
                        <Link href={`/admin/users/${user.id}`} className="block">
                          <Badge variant={statusVariant(statusValue)}>{statusValue}</Badge>
                        </Link>
                      </TableCell>
                      <TableCell>
                        <Link href={`/admin/users/${user.id}`} className="block text-muted-foreground">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex gap-2">
            {currentPage > 1 ? (
              <Button variant="outline" asChild>
                <Link href={`/admin/users?${buildQuery(searchParams, currentPage - 1)}`}>Previous</Link>
              </Button>
            ) : (
            ...
            {currentPage < totalPages ? (
              <Button asChild>
                <Link href={`/admin/users?${buildQuery(searchParams, currentPage + 1)}`}>Next</Link>
              </Button>
            ) : (
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Search, filter, and manage registered users.</p>
      </div>
