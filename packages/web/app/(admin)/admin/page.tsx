...
async function getMetrics() {
  ...
  const [quarantinedCount] = await db.select({ value: count() }).from(skills).where(eq(skills.status, 'quarantined'));
  const [bannedCount] = await db.select({ value: count() }).from(userStatus).where(eq(userStatus.status, 'banned'));
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-muted-foreground">Monitor moderation activity and high-risk changes.</p>
      </div>
          ...
          <CardContent className="space-y-2">
            {recentEvents.length === 0 ? (
              ...
              recentEvents.map((event) => (
                <div key={event.id} className="rounded-md border p-3 flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <p className="font-medium">{event.action}</p>
                    <p className="text-xs text-muted-foreground">{event.actorName ?? event.actorEmail ?? 'System'}</p>
                  </div>
          ...
          <CardContent className="space-y-2">
            {flaggedPackages.length === 0 ? (
              ...
              flaggedPackages.map((pkg) => (
                <div key={pkg.id} className="rounded-md border p-3 flex items-center justify-between gap-2">
                  <div>
                    <Link href={`/admin/packages/${encodeSkillName(pkg.name)}`} className="font-medium">
                      {pkg.name}
                    </Link>
                    {pkg.statusReason ? <p className="text-xs text-muted-foreground">{pkg.statusReason}</p> : null}
                  </div>
                  <Badge variant={pkg.status === 'quarantined' ? 'destructive' : 'outline'}>{pkg.status}</Badge>
                </div>
              ))
            )}
          </CardContent>
