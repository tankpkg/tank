...
export function PublisherBanDeleteButton({ packageName, publisherId }: { packageName: string; publisherId: string }) {
  ...
  const onBanAndDeleteAll = () => {
    ...
    startTransition(async () => {
      try {
        const response = await fetch(`/api/admin/packages/${encodeURIComponent(packageName)}/publisher-ban-delete`, {
          method: 'POST',
  ...
  return (
    <div className="mt-4 space-y-3 rounded border border-destructive/30 p-3">
      ...
      <Button type="button" variant="destructive" onClick={onBanAndDeleteAll} disabled={isPending}>
