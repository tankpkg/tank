...
function visibilityVariant(visibility: string | null): 'default' | 'secondary' | 'destructive' | 'outline' {
...
export default async function AdminPackageDetailPage({ params }: { params: Promise<{ name: string[] }> }) {
  const { name: nameSegments } = await params;
  const name = nameSegments
    .map((segment) => {
      try {
  ...
  const downloadCountResult = await db
    ...
    .where(
      and(eq(skillDownloadDaily.skillId, skill.id), gte(skillDownloadDaily.date, sql`CURRENT_DATE - INTERVAL '7 days'`))
    );
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        {skill.description ? <p className="mt-1 text-muted-foreground">{skill.description}</p> : null}
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <Badge variant={skill.featured ? 'default' : 'secondary'}>{skill.featured ? 'Yes' : 'No'}</Badge>
          </CardContent>
        </Card>
      </div>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-1 text-sm">
              <p>
                <span className="text-muted-foreground">Created:</span> {formatDate(skill.createdAt)}
              </p>
              <p>
                <span className="text-muted-foreground">Updated:</span> {formatDate(skill.updatedAt)}
              </p>
              <p>
                <span className="text-muted-foreground">ID:</span> <span className="font-mono text-xs">{skill.id}</span>
              </p>
            </CardContent>
          </Card>
        </div>
