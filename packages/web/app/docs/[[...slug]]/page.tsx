...
async function getRawContent(slug: string[]): Promise<string> {
  try {
    const filePath = join(process.cwd(), 'content/docs', slug.length > 0 ? `${slug.join('/')}.mdx` : 'index.mdx');
...
export default async function Page(props: { params: Promise<{ slug?: string[] }> }) {
  ...
  const breadcrumbs = page.slugs.map((_slug, index) => ({
    name: page.slugs.slice(0, index + 1).join('/'),
    url: `${BASE_URL}/docs/${page.slugs.slice(0, index + 1).join('/')}`
  }));

  const jsonLd = generateJsonLd(page.data.title, page.data.description, pageUrl, breadcrumbs);
