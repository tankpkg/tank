...
describe('GET /api/v1/search', () => {
  ...
  it('pagination works (page=2 with limit=1)', async () => {
    mockSearchSkills.mockResolvedValueOnce(makeSearchResponse([searchItems[1]], { page: 2, limit: 1, total: 2 }));
  ...
  it('passes tokens sort through to searchSkills', async () => {
    mockSearchSkills.mockResolvedValueOnce(makeSearchResponse([], { total: 0 }));

    const { GET } = await import('@/app/api/v1/search/route');
    await GET(makeGetRequest('http://localhost:3000/api/v1/search?q=test&sort=tokens'));
