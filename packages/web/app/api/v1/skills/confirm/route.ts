...
async function estimateTokenCountFromTarball(tarballPath: string): Promise<number | null> {
  try {
    const signed = await getStorageProvider().createSignedUrl(tarballPath, 3600);
    const downloadRes = await fetch(signed.signedUrl);
    if (!downloadRes.ok) {
      return null;
    }

    const compressed = new Uint8Array(await downloadRes.arrayBuffer());
    const decompressed = pako.ungzip(compressed);
    const tarBuffer = decompressed.buffer.slice(
    ...
    for (const entry of entries) {
      ...
      totalChars += decoder.decode(entry.data).length;
    }

    return Math.ceil(totalChars / 4);
  } catch (error) {
    console.warn('Token count estimation failed:', error);
    return null;
  }
}
...
export async function POST(request: Request) {
  ...
  const tokenCount = await estimateTokenCountFromTarball(version.tarballPath);
