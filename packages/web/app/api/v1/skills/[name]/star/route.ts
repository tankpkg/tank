...
async function hasSkillStarsTable(): Promise<boolean> {
  ...
  try {
    const result = (await db.execute(sql`SELECT to_regclass('public.skill_stars') IS NOT NULL AS "exists"`)) as Array<{
      exists: boolean;
    }>;
...
async function resolveSkillWithAccess(name: string, userId: string | null): Promise<string | null> {
...
export async function DELETE(request: Request, { params }: RouteParams) {
  try {
    ...
    await db.delete(skillStars).where(and(eq(skillStars.skillId, skillId), eq(skillStars.userId, userId)));
