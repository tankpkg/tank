...
export async function POST(request: Request) {
  try {
    ...
    if (!sessionCode || typeof sessionCode !== 'string') {
      return NextResponse.json({ error: 'Missing required field: sessionCode' }, { status: 400 });
    }
    ...
    if (!session) {
      ...
      return NextResponse.json({ error: 'Invalid, expired, or already used session code' }, { status: 400 });
    }
    ...
    if (!session.userId) {
      ...
      return NextResponse.json({ error: 'Session was not properly authorized' }, { status: 400 });
    }
