  ...
  return new ImageResponse(
    <div
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        <div style={{ fontSize: '48px', fontWeight: 'bold', lineHeight: 1.1 }}>{skillName}</div>
        {version && (
        ...
        <div style={{ fontSize: '22px', color: '#cbd5e1', lineHeight: 1.4 }}>{truncatedDesc}</div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
        <div
          ...
          <div style={{ fontSize: '24px', fontWeight: 'bold', color }}>{scoreLabel(auditScore)}</div>
        </div>
        <div style={{ fontSize: '16px', color: '#64748b' }}>tankpkg.dev</div>
      </div>
    </div>,
    { ...size }
  );
