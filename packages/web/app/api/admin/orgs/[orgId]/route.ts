...
import { auditEvents, skills } from '@/lib/db/schema';
...
export const DELETE = withAdminAuth(async (req: NextRequest, ctx: AdminAuthContext): Promise<NextResponse> => {
  ...
  const [memberCountResult] = await db.select({ count: count() }).from(member).where(eq(member.organizationId, orgId));

  await db.transaction(async (tx) => {
    await tx.update(skills).set({ orgId: null }).where(eq(skills.orgId, orgId));
