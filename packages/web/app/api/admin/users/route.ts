...
export const GET = withAdminAuth(async (req: NextRequest) => {
  ...
  if (search) {
    conditions.push(
      or(ilike(user.name, `%${search}%`), ilike(user.email, `%${search}%`), ilike(user.githubUsername, `%${search}%`))
    );
  }
  ...
  if (statusFilter && (statusFilter === 'active' || statusFilter === 'suspended' || statusFilter === 'banned')) {
    if (statusFilter === 'active') {
      statusConditions.push(or(sql`${latestStatusSubquery.status} IS NULL`, eq(latestStatusSubquery.status, 'active')));
    } else {
