...
describe('GET /api/admin/users', () => {
  ...
  it('returns paginated users with defaults', async () => {
    ...
    expect(data.users[1].latestStatus).toEqual(expect.objectContaining({ status: 'banned', reason: 'Spam' }));
  });
  ...
  it('respects search, role, and pagination params', async () => {
    ...
    const { GET } = await import('../route');
    const response = await GET(makeGetRequest('/api/admin/users?search=alice&role=admin&page=1&limit=10'));
...
describe('PATCH /api/admin/users/[userId]', () => {
  ...
  it('returns 400 for invalid role', async () => {
    const { PATCH } = await import('../[userId]/route');
    const response = await PATCH(makeRequest('/api/admin/users/user-1', 'PATCH', { role: 'superadmin' }));
  ...
  it('returns 400 for missing role', async () => {
    const { PATCH } = await import('../[userId]/route');
    const response = await PATCH(makeRequest('/api/admin/users/user-1', 'PATCH', {}));
    expect(response.status).toBe(400);
  });
  ...
  it('returns 400 when trying to change own role', async () => {
    const { PATCH } = await import('../[userId]/route');
    const response = await PATCH(makeRequest('/api/admin/users/admin-1', 'PATCH', { role: 'user' }));
    expect(response.status).toBe(400);
    const data = await response.json();
  ...
  it('returns 404 when target user not found', async () => {
    ...
    const { PATCH } = await import('../[userId]/route');
    const response = await PATCH(makeRequest('/api/admin/users/nonexistent', 'PATCH', { role: 'admin' }));
    expect(response.status).toBe(404);
  });
  ...
  it('promotes user to admin and logs audit event', async () => {
    ...
    const auditInsert = insertCalls.find((c) => (c.values as Record<string, unknown>)?.action === 'user.promote');
  ...
  it('demotes admin to user and logs audit event', async () => {
    ...
    const { PATCH } = await import('../[userId]/route');
    const response = await PATCH(makeRequest('/api/admin/users/user-2', 'PATCH', { role: 'user' }));

    expect(response.status).toBe(200);
    const data = await response.json();
    expect(data.newRole).toBe('user');

    const auditInsert = insertCalls.find((c) => (c.values as Record<string, unknown>)?.action === 'user.demote');
    expect(auditInsert).toBeTruthy();
  });
});
...
describe('POST /api/admin/users/[userId]/status', () => {
  ...
  it('returns 400 when banning without reason', async () => {
    const { POST } = await import('../[userId]/status/route');
    const response = await POST(makeRequest('/api/admin/users/user-1/status', 'POST', { status: 'banned' }));
    expect(response.status).toBe(400);
    const data = await response.json();
  ...
  it('unbans user without requiring reason', async () => {
    ...
    const { POST } = await import('../[userId]/status/route');
    const response = await POST(makeRequest('/api/admin/users/user-1/status', 'POST', { status: 'active' }));
  ...
  it('logs correct audit action for each status', async () => {
    ...
    const { POST } = await import('../[userId]/status/route');
    await POST(makeRequest('/api/admin/users/user-1/status', 'POST', { status: 'banned', reason: 'Spam' }));

    const auditInsert = insertCalls.find((c) => (c.values as Record<string, unknown>)?.action === 'user.ban');
    expect(auditInsert).toBeTruthy();
    expect(auditInsert?.values).toEqual(
      expect.objectContaining({
        action: 'user.ban',
