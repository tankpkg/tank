...
export const POST = withAdminAuth(
  async (req: NextRequest, { user: adminUser }: AdminAuthContext): Promise<NextResponse> => {
    ...
    if (validStatus === 'banned' && (!reason || (typeof reason === 'string' && reason.trim() === ''))) {
      return NextResponse.json({ error: 'Reason is required when banning a user' }, { status: 400 });
    }

    // Validate expiresAt if provided
    if (expiresAt !== undefined && expiresAt !== null) {
      if (typeof expiresAt !== 'string' || Number.isNaN(Date.parse(expiresAt))) {
        return NextResponse.json({ error: 'expiresAt must be a valid ISO datetime string' }, { status: 400 });
      }
    }
