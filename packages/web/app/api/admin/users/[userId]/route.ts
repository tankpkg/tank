...
export const GET = withAdminAuth(async (req: NextRequest): Promise<NextResponse> => {
  ...
  const [packagesResult] = await db.select({ count: count() }).from(skills).where(eq(skills.publisherId, userId));

  const [orgsResult] = await db.select({ count: count() }).from(member).where(eq(member.userId, userId));
...
export const PATCH = withAdminAuth(
  async (req: NextRequest, { user: adminUser }: AdminAuthContext): Promise<NextResponse> => {
    ...
    const parsed = userRoleSchema.safeParse(
      typeof body === 'object' && body !== null && 'role' in body ? (body as Record<string, unknown>).role : undefined
    );
    if (!parsed.success) {
      return NextResponse.json({ error: 'Invalid request body. Expected { role: "user" | "admin" }' }, { status: 400 });
    }
    ...
    if (oldRole === newRole) {
      return NextResponse.json({ error: `User already has role '${newRole}'` }, { status: 400 });
    }
