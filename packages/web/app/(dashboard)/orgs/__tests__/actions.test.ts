...
describe('org server actions', () => {
  ...
  describe('inviteMember', () => {
    ...
    it('throws when user is not authenticated', async () => {
      ...
      const { inviteMember } = await import('../actions');
      await expect(inviteMember({ organizationId: 'org-1', email: 'test@test.com', role: 'member' })).rejects.toThrow(
        'Unauthorized'
      );
    });
  });
  ...
  describe('removeMember', () => {
    ...
    it('throws when user is not authenticated', async () => {
      ...
      const { removeMember } = await import('../actions');
      await expect(removeMember({ organizationId: 'org-1', memberIdOrEmail: 'member-1' })).rejects.toThrow(
        'Unauthorized'
      );
    });
  });
