    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Create and manage API keys for CLI and programmatic access.</p>
        </div>
            ...
            {newTokenValue ? (
              ...
              <>
                <DialogHeader>
                  ...
                  <DialogDescription>Create a scoped token for your workflows.</DialogDescription>
                </DialogHeader>
      ...
      {loading ? (
        <div className="text-muted-foreground text-sm py-8 text-center">Loading tokens...</div>
      ) : tokens.length === 0 ? (
        <div className="text-center py-12 border rounded-lg">
          <p className="text-muted-foreground">No API tokens yet.</p>
          <p className="text-muted-foreground text-sm mt-1">Create a token to authenticate with the Tank CLI.</p>
        </div>
      ) : (
            ...
            <TableBody>
              {tokens.map((token) => (
                <TableRow key={token.id}>
                  ...
                  <TableCell>
                    <code className="text-sm bg-muted px-2 py-0.5 rounded">{getDisplayKey(token)}</code>
                  </TableCell>
                  <TableCell className="text-muted-foreground text-sm">{formatDate(token.createdAt)}</TableCell>
                  <TableCell className="text-muted-foreground text-sm">{formatDate(token.lastRequest)}</TableCell>
                  <TableCell>
                    {token.expiresAt ? (
