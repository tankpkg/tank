...
function FilePreview({
  ...
  if (!filePath) {
    return <p className="text-xs text-muted-foreground italic p-2">Click a file to preview</p>;
  }

  if (isLoading) {
    return <p className="text-xs text-muted-foreground italic p-2">Loading...</p>;
  }

  if (error) {
    return <p className="text-xs text-destructive italic p-2">{error}</p>;
  }
  ...
  return <p className="text-xs text-muted-foreground italic p-2">Preview not available.</p>;
}
...
export function FileExplorer({ files, skillName, version, readme, manifest }: FileExplorerProps) {
  ...
  if (files.length === 0) {
    return <p className="text-sm text-muted-foreground py-4">No files in this package.</p>;
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div className="border rounded-lg p-2">
        ...
        <FilePreview content={fileContent} isLoading={isLoading} error={error} filePath={selectedFile} />
      </div>
    </div>
  );
}
