...
Tank's install pipeline does significantly more than download a file. Every install resolves the full dependency tree, verifies SHA-512 integrity for every package, extracts tarballs through a security filter, checks your project's permission budget (prompting interactively to expand if needed), and writes a deterministic lockfile. This page covers every command and behavior you need for day-to-day skill management.
...
### Auto-accept permission expansions (CI mode)
...
When a skill requests permissions beyond your `tank.json` budget, Tank normally prompts you interactively. The `--yes` flag auto-accepts all budget expansions — useful for CI/CD pipelines and non-interactive environments where no TTY is available.
...
When one or more skills request permissions that exceed your `tank.json` budget, Tank does not hard-fail. Instead, it:

1. **Collects** all violations across all resolved skills
2. **Displays** a grouped summary showing each skill and its requested permissions
3. **Prompts** you to confirm the budget expansion
...
If you accept, Tank merges the requested permissions into your `tank.json` and writes the updated file to disk before continuing the install. If you decline, the install aborts cleanly.

Use `--yes` / `-y` to auto-accept in CI/CD pipelines. See [Permissions & Access Control](/docs/permissions) for full details on the permission model and budget enforcement.
...
# --yes auto-accepts any permission budget expansions
tank install --yes
```
