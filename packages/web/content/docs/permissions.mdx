...
<Callout type="info">
Tank permissions are a defense-in-depth layer on top of OS-level controls. They cannot enforce syscall-level restrictions without a sandbox (planned for Phase 3), but they create an auditable contract between skill publishers and project owners — and enforce it at install time.
</Callout>
...
Permissions are declared in the `permissions` object of a skill's `SKILL.md` manifest. The full schema is defined in `packages/shared/src/schemas/permissions.ts` as a Zod schema and validated on both the CLI and registry server.
...
<Callout type="warn">
Avoid `["*"]` or broad wildcard patterns. A skill that needs to reach any host on the internet should not be installed without careful review. The security scanner flags broad wildcard network permissions as `medium` severity.
</Callout>
...
Controls which paths the skill is allowed to read. Values are glob patterns evaluated relative to the **project root** (the directory containing `skills.json`).
...
<Callout type="error">
Never grant write access to `./` (project root), `../**` (parent directories), or any path containing configuration files (`.env`, `.git/**`, `package.json`). A skill with write access to `package.json` can inject malicious dependencies on the next `npm install`.
</Callout>
...
```yaml
permissions:
  subprocess: true   # allow shell execution
  # or
  subprocess: false  # deny shell execution (default)
```
...
<Callout type="error">
Treat `subprocess: true` as a red flag during skill review. Legitimate skills rarely need subprocess access. If you see it, verify exactly what commands the skill runs by inspecting its source code before installing.
</Callout>
...
Individual skill permissions declare what a skill *claims* to need. The permission budget in `skills.json` declares what your project *allows*. Every skill installed into your project must fit within the budget — if any skill claims more than the budget permits, installation fails with a detailed error showing which permission was exceeded and by which skill.

### Example `skills.json` with Budget
...
✗ Installation failed: permission budget exceeded
  ...
  Either:
    1. Add "*.googleapis.com" to the network.outbound budget in skills.json
...
The budget enforcement logic runs in the CLI (`packages/cli/src/lib/`) during `tank install` and `tank update`. It compares each skill's declared permissions against the project budget field by field.
...
The escalation logic is implemented in `packages/web/lib/permission-escalation.ts` and runs server-side during `tank publish`.
...
<Callout type="info">
These rules protect skill consumers. When you run `tank update`, you expect patch updates to be safe by definition — no new capabilities, no new attack surface. The escalation enforcement makes that guarantee hard rather than advisory.
</Callout>
