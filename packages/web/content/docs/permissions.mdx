...
Tank's permission model requires skills to declare upfront what they need. Projects then define a **permission budget** — a ceiling that every installed skill must fit within. If any skill claims more than the budget allows, Tank collects all violations and prompts you to review and approve the budget expansion before proceeding. You can auto-accept with `--yes` for CI environments, or decline to abort the install.
...
Individual skill permissions declare what a skill _claims_ to need. The permission budget in `tank.json` declares what your project _allows_. Every skill installed into your project must fit within the budget. If any skill claims more than the budget permits, Tank collects all violations and prompts you interactively to expand the budget. If you accept, `tank.json` is updated automatically and the install continues. If you decline, the install aborts with a detailed error showing which permissions were exceeded and by which skill.

Use `tank install --yes` (or `-y`) to auto-accept budget expansions in CI/CD pipelines and non-interactive environments.
...
If `@community/seo-audit` declares `network.outbound: ["*.googleapis.com"]` — a domain not in the budget — `tank install` will prompt you:

```
⚠ Permission budget exceeded — 1 skill requires permissions beyond your tank.json budget:

  @community/seo-audit@3.0.0
    network.outbound: *.googleapis.com

? Expand your permission budget in tank.json to allow these? (y/N)
```

If you accept, Tank merges `*.googleapis.com` into your `tank.json` `network.outbound` array and continues the install. If you decline (or the prompt times out), installation aborts.

In CI/non-interactive mode (`tank install --yes`), expansions are auto-accepted.
...
3. **No match:** Recorded as a violation for the interactive prompt
...
The enforcer checks all skills against the budget before extracting any of them. If multiple skills exceed the budget, all violations are collected and presented together in a single interactive prompt:

```
⚠ Permission budget exceeded — 2 skills require permissions beyond your tank.json budget:

  @foo/skill-a@1.0.0
    network.outbound: api.openai.com

  @bar/skill-b@2.3.1
    subprocess: true (currently disallowed)
    environment: AWS_SECRET_ACCESS_KEY

? Expand your permission budget in tank.json to allow these? (y/N)
```

If accepted, all requested permissions are merged into `tank.json` and the install proceeds. If declined, the install aborts. Use `--yes` to auto-accept in CI.
...
The escalation logic is implemented in `apps/web/lib/permission-escalation.ts` and runs server-side during `tank publish`.
