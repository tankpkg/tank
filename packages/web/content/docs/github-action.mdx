...
1. **Validates** your `skills.json` manifest — schema, semver format, and permission declarations
...
| `directory` | No | `.` | Directory containing `skills.json`. Use if your skill is not at the repo root. |
...
- `skills.json` is invalid or missing required fields
...
Tank enforces version immutability. Increment the version in your `skills.json` before pushing. Patch → `1.0.1`, minor → `1.1.0`, major → `2.0.0`.
