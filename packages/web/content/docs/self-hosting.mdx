...
```bash
docker compose exec web bun --filter=@internal/web drizzle-kit push
```

Or run as a one-shot migration container:

```bash
docker compose run --rm web bun --filter=@internal/web drizzle-kit push
```

### 5) Bootstrap the Admin User

```bash
docker compose exec web bun --filter=@internal/web admin:bootstrap
```
...
- **Scanner**: Security scanner code lives in `packages/scanner/`.
- **Turbo builds**: Use `bun turbo build --filter=@internal/web...` for dependency-aware monorepo builds.
...
```bash
bun turbo build --filter=@internal/web...
```
