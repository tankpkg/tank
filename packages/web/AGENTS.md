...
│           └── service-accounts/ # CI/CD API keys
├── api-python/                   # Vercel Python stubs — mirrors apps/scanner/
...
| Modify security scanning | `api-python/analyze/`                     | Must sync with `apps/scanner/` at root |
...
```bash
bun test --filter=web                    # All web tests
bun test --filter=web -- publish.test.ts # Specific test
bun test:perf                            # Performance tests (needs real DB)
```
