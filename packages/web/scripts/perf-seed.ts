 ...
 *
 * Usage:
 *   bun --filter=@internal/web run perf:seed
...
async function insertSkillsAndVersions(sql: postgres.Sql) {
  ...
  for (const skill of skillDefs) {
    ...
    for (let v = 0; v < VERSIONS_PER_SKILL; v++) {
      ...
      const integrityHash = createHash('sha512').update(`${skill.name}@${version}`).digest('base64');
      ...
      const manifest = JSON.stringify({
        ...
        license: 'MIT',
        files: ['SKILL.md', 'index.ts', 'README.md', 'package.json', 'lib/helpers.ts'],
        permissions: {
