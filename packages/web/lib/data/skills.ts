...
export type SortOption = "updated" | "downloads" | "stars" | "security" | "tokens" | "name";
 ...
 */
export async function getSkillDetail(name: string): Promise<SkillDetailResult | null> {
  ...
  const rows = (await db.execute(sql`
    SELECT
      ...
      sv.version,
      sv.integrity,
      sv.token_count AS "versionTokenCount",
      sv.permissions,
      sv.manifest,
  ...
  const latestVersion: SkillVersionDetail | null = latestRow
    ? {
        ...
        tokenCount: latestRowData?.versionTokenCount != null ? Number(latestRowData.versionTokenCount) : null,
...
function buildPrimarySort(sort: SortOption, starsAvailable: boolean) {
  switch (sort) {
    ...
    case "tokens":
      return sql`(sv.token_count IS NULL) ASC, coalesce(sv.token_count, 0) DESC`;
    case "name":
...
export async function searchSkills(
  ...
  const viewerUserId = params.requesterUserId !== undefined ? params.requesterUserId : await resolveViewerUserId();
  const starsTableAvailable = await hasSkillStarsTable();
  const visClause = visibilityClause(viewerUserId);
