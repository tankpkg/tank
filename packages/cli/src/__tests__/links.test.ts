...
describe('writeLinks', () => {
  ...
  it('sorts agent link keys alphabetically', () => {
    ...
    const raw = fs.readFileSync(path.join(tmpDir, 'links.json'), 'utf-8');
    const parsed = JSON.parse(raw) as LinksManifest;
    expect(Object.keys(parsed.links['alpha-skill'].agentLinks)).toEqual(['alpha', 'middle', 'zeta']);
  });
...
describe('readLocalLinks', () => {
  ...
  it('returns manifest from .tank/links.json', () => {
    ...
    fs.writeFileSync(path.join(tankDir, 'links.json'), `${JSON.stringify(manifest, null, 2)}\n`);
...
describe('addLink', () => {
  ...
  it('replaces existing skill entry', () => {
    ...
    const updated = addLink(manifest, 'replace-skill', sampleEntry({ sourceDir: '/tmp/new' }));
