...
describe('loginCommand', () => {
  ...
  it('handles start endpoint failure', async () => {
    ...
    await expect(loginCommand({ configDir: tmpDir, timeout: 2000 })).rejects.toThrow('Failed to start auth session');
  ...
  it('handles exchange endpoint server error (non-400)', async () => {
    ...
    await expect(loginCommand({ configDir: tmpDir, timeout: 2000 })).rejects.toThrow('Exchange failed');
  ...
  it('times out when authorization never completes', async () => {
    ...
    await expect(loginCommand({ configDir: tmpDir, timeout: 100, pollInterval: 10 })).rejects.toThrow(
      'Login timed out'
    );
  ...
  it('prints URL manually when browser fails to open', async () => {
    ...
    const allOutput = [...logSpy.mock.calls, ...errorSpy.mock.calls].map((c) => c.join(' ')).join('\n');
  ...
  it('opens browser with authUrl', async () => {
    ...
    expect(openMock).toHaveBeenCalledWith('https://tankpkg.dev/cli-login?session=sess-open');
  ...
  it('handles start endpoint non-JSON error body with statusText fallback', async () => {
    ...
    await expect(loginCommand({ configDir: tmpDir, timeout: 2000 })).rejects.toThrow(
      'Failed to start auth session: Internal Server Error'
    );
  ...
  it('uses custom registry URL from config', async () => {
    ...
    fs.writeFileSync(path.join(tmpDir, 'config.json'), JSON.stringify({ registry: 'https://custom.example.com' }));
  ...
  it('handles exchange non-400 error with non-JSON body', async () => {
    ...
    await expect(loginCommand({ configDir: tmpDir, timeout: 2000 })).rejects.toThrow(
      'Exchange failed: Internal Server Error'
    );
