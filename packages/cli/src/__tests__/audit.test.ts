...
describe('auditCommand', () => {
  ...
  function writeLockfile(skills: Record<string, unknown>) {
    fs.writeFileSync(path.join(projectDir, 'skills.lock'), JSON.stringify({ lockfileVersion: 1, skills }, null, 2));
  }
  ...
  it('throws on network error', async () => {
    ...
    await expect(auditCommand({ configDir })).rejects.toThrow(/network/i);
  });
  ...
  it('calls correct API URL with encoded skill name', async () => {
    ...
    mockFetch.mockResolvedValueOnce(new Response(JSON.stringify(makeVersionResponse()), { status: 200 }));
  ...
  it('throws on network error when auditing specific skill', async () => {
    ...
    await expect(auditCommand({ name: '@test/skill-a', configDir })).rejects.toThrow(/network/i);
  });
  ...
  it('throws on API error when auditing specific skill', async () => {
    ...
    await expect(auditCommand({ name: '@test/skill-a', configDir })).rejects.toThrow(/API error/i);
  });
});
