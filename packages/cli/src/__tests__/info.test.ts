...
describe('infoCommand', () => {
  ...
  function setupSuccessfulInfo() {
    ...
    mockFetch.mockResolvedValueOnce(new Response(JSON.stringify(versionDetails), { status: 200 }));
  }
  ...
  it('handles missing optional fields gracefully', async () => {
    ...
    mockFetch.mockResolvedValueOnce(new Response(JSON.stringify(minimalMeta), { status: 200 }));
    mockFetch.mockResolvedValueOnce(new Response(JSON.stringify(minimalVersion), { status: 200 }));
  ...
  it('handles network errors', async () => {
    ...
    await expect(infoCommand({ name: '@vercel/next-skill', configDir })).rejects.toThrow(/network/i);
  });
  ...
  it('uses statusText when non-200 non-404 response has non-JSON body', async () => {
    ...
    await expect(infoCommand({ name: '@vercel/next-skill', configDir })).rejects.toThrow(
      /Internal Server Error|Failed to fetch/
    );
  });
  ...
  it('shows visibility field when present in metadata', async () => {
    ...
    mockFetch.mockResolvedValueOnce(new Response(JSON.stringify(metaWithVisibility), { status: 200 }));
    mockFetch.mockResolvedValueOnce(new Response(JSON.stringify(versionDetails), { status: 200 }));
