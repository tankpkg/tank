...
describe('linker', () => {
  ...
  describe('linkSkillToAgents', () => {
    it.skipIf(isWindows)('creates symlinks in each detected agent dir', () => {
    ...
    it.skipIf(isWindows)('skips when symlink already points to correct target', () => {
      linkSkillToAgents({
        skillName: '@tank/test-skill',
        sourceDir: skillSource,
        linksDir,
    ...
    it.skipIf(isWindows)('overwrites symlink when it points to wrong target', () => {
      const { claude } = getAgentSkillDirs(fakeHome);
    ...
    it.skipIf(isWindows)('recreates dangling symlinks', () => {
    ...
    it.skipIf(isWindows)('does not delete real directories when path exists', () => {
      const { claude } = getAgentSkillDirs(fakeHome);
    ...
    it.skipIf(isWindows)('returns failure on permission error without throwing', () => {
    ...
    it.skipIf(isWindows)('updates links.json manifest after linking', () => {
      linkSkillToAgents({
        skillName: '@tank/test-skill',
        sourceDir: skillSource,
        linksDir,
    ...
    it.skipIf(isWindows)('returns correct counts for linked, skipped, and failed', () => {
      const { claude } = getAgentSkillDirs(fakeHome);
      fs.mkdirSync(path.join(claude, 'tank--test-skill'), { recursive: true });
  ...
  describe('unlinkSkillFromAgents', () => {
    it.skipIf(isWindows)('removes symlinks and updates manifest', () => {
      linkSkillToAgents({
        skillName: '@tank/test-skill',
        sourceDir: skillSource,
        linksDir,
    ...
    it.skipIf(isWindows)('does not delete real directories', () => {
      linkSkillToAgents({
        skillName: '@tank/test-skill',
        sourceDir: skillSource,
        linksDir,
    ...
    it.skipIf(isWindows)('handles missing symlinks gracefully', () => {
      linkSkillToAgents({
        skillName: '@tank/test-skill',
        sourceDir: skillSource,
        linksDir,
  ...
  describe('getSkillLinkStatus', () => {
    it.skipIf(isWindows)('reports linked skills as valid', () => {
      linkSkillToAgents({
        skillName: '@tank/test-skill',
        sourceDir: skillSource,
        linksDir,
    ...
    it.skipIf(isWindows)('reports broken symlinks correctly', () => {
      const { claude } = getAgentSkillDirs(fakeHome);
      fs.mkdirSync(claude, { recursive: true });
