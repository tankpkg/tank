...
describe('updateCommand', () => {
  ...
  function writeGlobalLockfile(data: Record<string, unknown> = baseGlobalLockfile) {
    ...
    fs.writeFileSync(path.join(tankDir, 'skills.lock'), `${JSON.stringify(data, null, 2)}\n`);
  }
  ...
  it('prints "Already at latest" when no newer version', async () => {
    ...
    expect(vi.mocked(logger.info)).toHaveBeenCalledWith(expect.stringMatching(/already.*latest|up.to.date/i));
  });
  ...
  it('prints summary after updating all', async () => {
    ...
    expect(vi.mocked(logger.success)).toHaveBeenCalledWith(expect.stringMatching(/updated 2 skill/i));
  });
  ...
  it('handles mix of up-to-date and outdated skills', async () => {
    ...
    expect(vi.mocked(installCommand)).toHaveBeenCalledWith(expect.objectContaining({ name: 'simple-skill' }));

    // Summary should say 1 updated
    expect(vi.mocked(logger.success)).toHaveBeenCalledWith(expect.stringMatching(/updated 1 skill/i));
  });
  ...
  it('prints "All skills up to date" when nothing to update', async () => {
    ...
    expect(vi.mocked(logger.info)).toHaveBeenCalledWith(expect.stringMatching(/all skills up to date/i));
  });
  ...
  it('errors when global lockfile is missing', async () => {
    ...
    await expect(updateCommand({ configDir, global: true, homedir })).rejects.toThrow(/skills\.lock|global/i);
  });
  ...
  it('errors when skills.json is corrupt (invalid JSON)', async () => {
    ...
    fs.writeFileSync(path.join(tmpDir, 'skills.json'), '{ invalid json content }');
  ...
  it('logs message when updating all skills with empty skills map', async () => {
    ...
    expect(vi.mocked(logger.info)).toHaveBeenCalledWith(expect.stringMatching(/no skills/i));
  });
});
