...
function makeMeta(name: string, version: string, dependencies: Record<string, string> = {}): RegistrySkillMeta {
  return {
    name,
    version,
...
describe('resolveDependencyTree', () => {
  ...
  it('deduplicates diamond dependencies when both parents require same range', async () => {
    ...
    const graph = await resolveDependencyTree({ A: '^1.0.0', B: '^1.0.0' }, fetcher);

    expect(graph.nodes.size).toBe(3);
    expect(graph.nodes.get('C')?.version).toBe('1.2.0');
    const cMetaCalls = fetchMetadata.mock.calls.filter(([name]) => name === 'C');
  ...
  it('sorts installOrder alphabetically by skillKey', async () => {
    ...
    const graph = await resolveDependencyTree({ zebra: '^1.0.0', alpha: '^2.0.0', middle: '^3.0.0' }, fetcher);

    expect(graph.installOrder).toEqual(['alpha@2.0.0', 'middle@3.0.0', 'zebra@1.0.0']);
  });
  ...
  it('throws Version conflict when dependency ranges are incompatible', async () => {
    ...
    await expect(resolveDependencyTree({ A: '^1.0.0', B: '^1.0.0' }, fetcher)).rejects.toThrow(
  ...
  it('throws when no available versions exist for a required skill', async () => {
    ...
    await expect(resolveDependencyTree({ foo: '^1.0.0' }, fetcher)).rejects.toThrow(/Version conflict for foo/);
  });
  ...
  it('throws when no available version satisfies the requested range', async () => {
    ...
    await expect(resolveDependencyTree({ foo: '^2.0.0' }, fetcher)).rejects.toThrow(/Version conflict for foo/);
  });
  ...
  it('calls fetchMetadata exactly once per (name, version)', async () => {
    ...
    const cCalls = fetchMetadata.mock.calls.filter(([name, version]) => name === 'C' && version === '1.0.0');
  ...
  it('calls fetchVersions exactly once per skill name', async () => {
    ...
    const countsBySkill = fetchVersions.mock.calls.reduce(
      (acc: Record<string, number>, [name]: unknown[]) => {
