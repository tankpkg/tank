...
describe('searchCommand', () => {
  ...
  it('handles network errors', async () => {
    ...
    await expect(searchCommand({ query: 'next', configDir })).rejects.toThrow(/network/i);
  });
  ...
  it('truncates long descriptions', async () => {
    ...
    mockFetch.mockResolvedValueOnce(new Response(JSON.stringify(longDescResults), { status: 200 }));
  ...
  it('handles non-200 API responses', async () => {
    ...
    await expect(searchCommand({ query: 'next', configDir })).rejects.toThrow();
  });
  ...
  it('color-codes scores: green for high, yellow for medium, red for low', async () => {
    ...
    mockFetch.mockResolvedValueOnce(new Response(JSON.stringify(mixedScoreResults), { status: 200 }));
  ...
  it('non-200 with non-JSON body uses statusText fallback', async () => {
    ...
    await expect(searchCommand({ query: 'test', configDir })).rejects.toThrow('Internal Server Error');
  });
  ...
  it('single result uses singular "skill" in count', async () => {
    ...
    mockFetch.mockResolvedValueOnce(new Response(JSON.stringify(singleResult), { status: 200 }));
