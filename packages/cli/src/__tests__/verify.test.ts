...
describe('verifyCommand', () => {
  ...
  function writeLockfile(skills: SkillsLock['skills']) {
    ...
    fs.writeFileSync(path.join(tmpDir, 'skills.lock'), `${JSON.stringify(lock, null, 2)}\n`);
  }
  ...
  it('passes when all skills are installed correctly', async () => {
    ...
    expect(logger.success).toHaveBeenCalledWith(expect.stringMatching(/2 skills verified/i));
  });
  ...
  it('fails when skill directory is missing', async () => {
    ...
    await expect(verifyCommand({ directory: tmpDir })).rejects.toThrow(/verification failed/i);

    expect(logger.error).toHaveBeenCalledWith(expect.stringMatching(/missing-skill/));
  });

  it('fails when lockfile is missing', async () => {
    // No lockfile written
    await expect(verifyCommand({ directory: tmpDir })).rejects.toThrow(/skills\.lock/i);
  });
  ...
  it('works with scoped packages (@org/skill)', async () => {
    ...
    expect(logger.success).toHaveBeenCalledWith(expect.stringMatching(/1 skill verified/i));
  });
  ...
  it('passes when lockfile has empty skills object', async () => {
    ...
    expect(logger.success).toHaveBeenCalledWith(expect.stringMatching(/no skills to verify/i));
  });
  ...
  it('fails when skill directory exists but is empty', async () => {
    ...
    expect(logger.error).toHaveBeenCalledWith(expect.stringMatching(/empty-skill.*empty/i));
  });
  ...
  it('error message includes issue count', async () => {
    ...
    await expect(verifyCommand({ directory: tmpDir })).rejects.toThrow(/3 issues/i);
  });
  ...
  it('throws when lockfile key is invalid (no version separator)', async () => {
    ...
    await expect(verifyCommand({ directory: tmpDir })).rejects.toThrow(/invalid lockfile key/i);
  });
});
