...
describe('doctorCommand', () => {
  ...
  it.skipIf(isWindows)('reports local skills with linked status', async () => {
    fs.mkdirSync(path.join(fakeHome, '.claude'), { recursive: true });
  ...
  it.skipIf(isWindows)('reports dev-linked skills', async () => {
    fs.mkdirSync(path.join(fakeHome, '.claude'), { recursive: true });
  ...
  it.skipIf(isWindows)('reports broken symlinks with warning', async () => {
    fs.mkdirSync(path.join(fakeHome, '.claude'), { recursive: true });
  ...
  it.skipIf(isWindows)('provides actionable suggestions for broken links', async () => {
    fs.mkdirSync(path.join(fakeHome, '.claude'), { recursive: true });
    const skillName = '@tank/react';
    writeSkillsJson(projectDir, { [skillName]: '^1.0.0' });
