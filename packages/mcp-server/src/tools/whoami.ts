...
export function registerWhoamiTool(server: McpServer): void {
  server.tool(
    'whoami',
    'Show the authenticated Tank user for the current local session.',
    {},
    async () => {
      ...
      if (authCheck.reason === 'network-error') {
        return {
          content: [{ type: 'text' as const, text: `Failed to connect to the registry. Check your network connection.\nError: ${authCheck.error ?? 'unknown'}` }],
          isError: true,
        };
      }
