...
- **Harness**: Vitest-based tests located in `packages/web/tests/perf/`.
- **Budgets**: Defined in `packages/web/perf/budgets.json`.
- **Scripts**:
...
- **Artifacts**: Results are saved to `packages/web/perf/results/*.json`. `latest.json` always points to the most recent run.
...
1. **Seed the database**:
   ```bash
   bun --filter=web run perf:seed
   ```
2. **Run the performance suite**:
   ```bash
   bun run test:perf
   ```
   _Note: This will build the app and start a production server on port 3999._
3. **Generate a report**:
   ```bash
   bun --filter=web run perf:report
   ```
...
Run `bun run perf:report` (or check the CI log for the "Generate performance report" step). Identify which route and which metric failed.
...
Look at `packages/web/perf/results/latest.json`. Compare the `aggregated` values with previous runs (available in CI artifacts or your local history).
