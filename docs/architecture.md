...
- `packages/cli` → `tank` command: init, install, publish, scan, verify, link → depends on `@internals/schemas`
- `packages/mcp-server` → MCP server with CLI-parity tools over stdio → depends on `@internals/schemas`
- `apps/registry` → TanStack Start registry app (migration target) → depends on `@internals/schemas`
- `apps/registry-legacy` → Next.js registry app (maintained during cutover) → depends on `@internals/schemas`
- `packages/internals-schemas` → Zod schemas, types, constants, semver helpers → depends on no internal packages
- `packages/internals-helpers` → pure shared helpers → depends on no internal packages
- `apps/python-api` → Python scan service and 6-stage pipeline → depends on no TS packages

Hard rule: CLI, MCP, and registry apps never import each other. `@internals/*` is the only TS cross-package contract. Scanner communicates over HTTP, not imports.
...
internals-schemas <- registry
internals-schemas <- registry-legacy
python-api <-http- registry / registry-legacy
```

## Web Apps

Two registry surfaces are active during migration:

- `apps/registry` — TanStack Start (primary migration target). Routes: TanStack Router file-based. API: Hono in `src/api/`. Server fns: `src/query/`. Auth: Better Auth in `src/lib/auth/`.
- `apps/registry-legacy` — Next.js (maintained during cutover). App Router serves registry pages, dashboard pages, admin pages, and API routes.
...
- `apps/registry-legacy/lib/db.ts` provides the cached `postgres` + Drizzle singleton.
- `apps/registry-legacy/lib/db/schema.ts` defines domain tables:
  - `skills`
  - `skill_versions`
  - `skill_access`
  - `service_accounts`
  ...
  - `scan_results`
  - `scan_findings`
- `apps/registry-legacy/lib/db/auth-schema.ts` is generated by better-auth. Never edit it.

Storage is abstracted behind `apps/registry-legacy/lib/storage/`.
...
API key validation lives in `apps/registry-legacy/lib/auth-helpers.ts`.
...
`packages/internals-schemas` is a pure library.
