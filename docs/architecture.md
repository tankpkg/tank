...
- `apps/registry` → TanStack Start registry app → depends on `@internals/schemas`
...
python-api <-http- registry
```

## Web Apps

- `apps/registry` — TanStack Start. Routes: TanStack Router file-based. API: Hono in `src/api/`. Server fns: `src/query/`. Auth: Better Auth in `src/lib/auth/`.
...
- `apps/registry/src/lib/db/` provides the Drizzle singleton and schema.
- `apps/registry/src/lib/db/schema.ts` defines domain tables:
  - `skills`
  - `skill_versions`
  - `skill_access`
  - `service_accounts`
  ...
  - `scan_results`
  - `scan_findings`
- `apps/registry/src/lib/db/auth-schema.ts` is generated by better-auth. Never edit it.

Storage is abstracted behind `apps/registry/src/services/storage/`.
...
API key validation lives in `apps/registry/src/lib/auth/tokens.ts`.
