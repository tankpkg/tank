...
import { apikey, user, organization } from '@/lib/db/auth-schema';
...
function normalizeScopes(raw: unknown): string[] {
  ...
  const scopes = Array.from(new Set(raw
  ...
  return NextResponse.json({
    serviceAccounts: accounts.map((account) => ({
      ...
      keys: keys
        ...
        .map((key) => ({
          ...key,
          scopes: typeof key.permissions === 'string' && key.permissions.trim().length > 0
...
export const POST = withAdminAuth(async (req: NextRequest, { user: adminUser }: AdminAuthContext): Promise<NextResponse> => {
  ...
  const keyExpiresInDays = typeof expiresInDays === 'number' && Number.isFinite(expiresInDays)
  ...
  const created = await db.transaction(async (tx) => {
    ...
    const [serviceAccount] = await tx.insert(serviceAccounts).values({
  ...
  const keyResult = await auth.api.createApiKey({
    body: {
      name: typeof keyName === 'string' && keyName.trim().length > 0
        ? keyName.trim()
        : `${displayName.trim()} key`,
