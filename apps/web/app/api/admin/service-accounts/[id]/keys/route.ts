...
export const POST = withAdminAuth(async (req: NextRequest): Promise<NextResponse> => {
  ...
  const [serviceAccount] = await db
    .select({ id: serviceAccounts.id, userId: serviceAccounts.userId, displayName: serviceAccounts.displayName, disabled: serviceAccounts.disabled })
  ...
  const keyResult = await auth.api.createApiKey({
    body: {
      name: typeof keyName === 'string' && keyName.trim().length > 0
        ? keyName.trim()
        : `${serviceAccount.displayName} key`,
