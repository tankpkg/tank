...
export async function POST(request: Request) {
  ...
  if (!verified) {
    return NextResponse.json({ error: 'Insufficient API key scope. Required: skills:publish' }, { status: 403 });
  }
  ...
  if (scopeMatch) {
    ...
    if (members.length === 0) {
      return NextResponse.json({ error: `You are not a member of org '${orgSlug}'` }, { status: 403 });
    }
  ...
  if (existingVersions.length > 0) {
    return NextResponse.json({ error: `Version ${version} already exists for ${name}` }, { status: 409 });
  }
