...
export function ForceDeleteCard({ packageName }: { packageName: string }) {
  ...
  const onForceDelete = () => {
    ...
    const secondConfirm = window.confirm('Final confirmation: this action is permanent and cannot be undone. Delete now?');
    if (!secondConfirm) {
      return;
    }
    ...
    startTransition(async () => {
      try {
        const response = await fetch(
          `/api/admin/packages/${encodeURIComponent(packageName)}?force=true`,
          {
            method: 'DELETE',
