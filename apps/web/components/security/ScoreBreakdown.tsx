      ...
      <div className="space-y-2">
        {criteria.map((criterion, index) => (
          ...
          >
            ...
            <span className={`text-sm font-medium ${criterion.passed ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
        ...
        <div className="mt-4 p-3 rounded-lg bg-purple-100 dark:bg-purple-900/30 border border-purple-200 dark:border-purple-800">
          ...
          {llmAnalysis.enabled && (
            <div className="text-xs text-purple-600 dark:text-purple-400 space-y-1">
              <div>Mode: {llmAnalysis.mode === 'byollm' ? 'Custom LLM' : llmAnalysis.mode === 'builtin' ? 'Built-in (Groq/OpenRouter)' : llmAnalysis.mode}</div>
              ...
              {llmAnalysis.findings_dismissed !== undefined && llmAnalysis.findings_dismissed > 0 && (
                <div className="text-green-600 dark:text-green-400">False positives dismissed: {llmAnalysis.findings_dismissed}</div>
              )}
              {llmAnalysis.findings_confirmed !== undefined && llmAnalysis.findings_confirmed > 0 && (
                <div className="text-red-600 dark:text-red-400">Threats confirmed: {llmAnalysis.findings_confirmed}</div>
              )}
            </div>
          )}
          {!llmAnalysis.enabled && (
