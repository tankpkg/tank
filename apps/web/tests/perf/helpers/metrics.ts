 ...
 */
export async function measureWebRoute(baseUrl: string, route: string): Promise<WebMetrics> {
  ...
  if (!response.ok) {
    throw new Error(`Web route ${route} returned ${response.status}: ${response.statusText}`);
  }
...
export async function measureApiRoute(baseUrl: string, route: string): Promise<number> {
  const url = `${baseUrl}${route}`;
  ...
  if (!response.ok && response.status !== 404) {
    throw new Error(`API route ${route} returned ${response.status}: ${response.statusText}`);
  }
