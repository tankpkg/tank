...
- [ ] `skills.json` exists at the root of your skill directory
...
The `skills.json` file is the single source of truth for your skill's identity, version, and permission declarations. It is named `skills.json` — **not** `tank.json`.
...
3. The `name` field in `skills.json` must match the org scope exactly.
...
- The declared permissions in the registry match your `skills.json`
...
- **Pin your own dependencies** — if your skill depends on other skills, pin them to exact versions in your `skills.json` to prevent your users from being affected by upstream updates.
...
### `skills.json` validation failure

```
ERROR  Invalid manifest: "name" must be scoped (e.g. "@org/skill-name")
```

Fix the `skills.json` field indicated in the error message, then re-run `tank publish --dry-run`.
