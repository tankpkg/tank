...
This resolves the latest published version that satisfies `*`, downloads and verifies the tarball, extracts it to `.tank/skills/@org/skill-name/`, and writes the resolved version and SHA-512 integrity hash to `skills.lock`.
...
When a `skills.lock` file already exists in your project, running `tank install` with no arguments performs a **deterministic install** — equivalent to `npm ci`. It installs exactly the versions and integrity hashes recorded in the lockfile, ignoring `skills.json` version ranges entirely.

```bash
# Deterministic install — uses skills.lock, not skills.json ranges
tank install
```
...
Tank uses a **fixpoint iteration algorithm** to resolve the full dependency tree. This means it doesn't just install the skill you named — it walks every skill's own `skills.json` to discover transitive dependencies, then iterates until the resolved set stabilizes.

### How it works

1. **Seed** — Start with the skills declared in your `skills.json`.
...
## The Lockfile (`skills.lock`)

The `skills.lock` file is the source of truth for your install state. Commit it to version control.
...
```json
{
  ...
  "skills": {
    "@org/skill-name@1.2.3": {
      "version": "1.2.3",
      "resolved": "https://tankpkg.com/api/v1/skills/@org/skill-name/1.2.3",
    ...
    "@shared/dep@1.4.2": {
      "version": "1.4.2",
      "resolved": "https://tankpkg.com/api/v1/skills/@shared/dep/1.4.2",
...
- **Deterministic install mode** — When `skills.lock` is present and `tank install` is run with no arguments, Tank performs a lockfile-only install. It does not resolve version ranges from `skills.json`. This is by design — CI pipelines should always use this mode.

<Callout type="info">
  Always commit `skills.lock` to version control. It is the guarantee that every developer and every CI run installs exactly the same code. Excluding it from git means losing reproducibility.
</Callout>
...
`tank update` re-resolves the version range from `skills.json`, downloads the new tarball, verifies integrity, and updates `skills.lock`. It does **not** upgrade beyond the declared version range — use `tank install @org/skill-name '^2.0.0'` to change the range.
...
1. Removes the entry from `skills.json`
2. Removes the skill and all its exclusive transitive dependencies from `skills.lock`
...
```bash
# Recompute SHA-512 for every installed file and compare against skills.lock
tank verify
...
```bash
# Uses skills.lock exactly — equivalent to npm ci
tank install
```
...
Use exact version pins (`1.2.3`) for skills in production agents. Use ranges (`^1.2.0`) in development where you want compatible updates. The lockfile provides determinism regardless of which you choose, but exact pins make your `skills.json` intent explicit.
