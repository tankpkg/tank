...
2. Store the key as `TANK_TOKEN` in your CI secrets
...
Service accounts are created by admins via the Tank API:
...
Add the API key as a CI secret named `TANK_TOKEN`.
...
- **Key rotation**: Set `expiresInDays` (max 365) and rotate before expiry
...
Token is invalid or expired. Create a new API key for the service account.
