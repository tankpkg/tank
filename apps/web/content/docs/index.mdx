...
|-----------|-------------|
| **SHA-512 integrity** | Every tarball is verified against a cryptographic hash in `skills.lock` before extraction |
| **Mandatory security scanning** | 6-stage pipeline (ingest → structure → static → injection → secrets → supply chain) runs on publish |
| **Permission declarations** | Skills declare what they need in `skills.json`; installation fails if a skill exceeds your budget |
| **Deterministic lockfile** | `skills.lock` pins exact versions and hashes — same behavior as `npm ci`, reproducible everywhere |
...
3. **[Publishing Reference](/docs/publishing)** — `skills.json` manifest, versioning, semver rules
...
| [Publishing](/docs/publishing) | `skills.json` manifest reference, versioning, and publish workflow |
