...
<Callout type="info">
  pnpm is required because Tank uses pnpm workspaces internally and its lockfile format. If you only have npm, install pnpm first: `npm i -g pnpm`.
</Callout>
...
For a local project install (stored in `.tank/skills/` relative to your working directory), omit the `-g` flag. Tank writes the resolved version and SHA-512 integrity hash to `skills.lock` — making future installs fully deterministic.
...
`tank permissions` aggregates all declared permissions across your installed skills and shows you what your agent is allowed to do. If any skill claims permissions outside your project's permission budget (defined in `skills.json`), installation will have already failed — but auditing afterward confirms the resolved state.

<Callout type="info">
  `tank verify` recomputes SHA-512 hashes of all installed files and compares them against `skills.lock`. A failed verify means files were modified on disk after install — treat this as a security event.
</Callout>
...
`tank init` runs an interactive prompt that generates a valid `skills.json` manifest with your skill's name, version, description, and permission declarations.
...
1. Check that `https://tankpkg.com` is reachable from your network.
...
Ensure your global pnpm or npm bin directory is on your `PATH`. Run `pnpm bin -g` or `npm bin -g` to find the directory, then add it to your shell profile.
