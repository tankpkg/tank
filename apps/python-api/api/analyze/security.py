...
CRITICAL_PATTERNS = [
    (
        r"ignore\s+(all|any)?\s*(previous|prior|above|earlier)\s*(instructions?|prompts?|rules?)",
        "prompt_injection",
        "Instruction override attempt",
    ),
    (
        r"<(system|human|assistant|tool_use|function_calls?|antml:)[^>]*>",
        "format_injection",
        "Claude XML control tag injection",
    ),
    (
...
HIGH_PATTERNS = [
    ...
    (
        r"(output|print|display|show|reveal)\s+(the|your|all)\s*(system\s+prompt|instructions?)",
        "exfiltration",
