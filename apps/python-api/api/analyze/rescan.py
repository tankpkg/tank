...
def verify_cron_auth(authorization: str | None) -> None:
    ...
    if vercel_env == "production":
        if not CRON_SECRET:
            raise HTTPException(status_code=500, detail="CRON_SECRET not configured in production")
...
async def update_version_audit_status(version_id: str, verdict: ScanVerdict, old_verdict: str | None) -> None:
...
async def generate_signed_url(tarball_path: str) -> str | None:
...
@app.post("/rescan")
async def rescan_handler(authorization: str | None = Header(None)) -> RescanResponse:
