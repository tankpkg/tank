...
def stage_token_analyze(ingest_result: IngestResult) -> StageResult:
    ...
    # Find node binary — needed for local module invocation
    node_bin = shutil.which("node")
    if not node_bin:
        # Vercel Python runtime has node at non-standard paths
        for candidate in [
            "/vercel/node-wrapper/node",
            "/usr/local/bin/node",
            "/opt/node/bin/node",
        ]:
            if os.path.isfile(candidate):
