...
class TestSnykScannerAvailable:
    ...
    def test_not_available_on_timeout(self, mock_run):
        mock_run.side_effect = subprocess.TimeoutExpired(cmd="uvx", timeout=30)
        assert is_snyk_scanner_available() is False
...
class TestRunSnykScanner:
    ...
    @patch("lib.scan.snyk_scanner.is_snyk_scanner_available")
    @patch("lib.scan.snyk_scanner.subprocess.run")
    def test_returns_empty_on_no_output(self, mock_run, mock_available):
    ...
    @patch("lib.scan.snyk_scanner.is_snyk_scanner_available")
    @patch("lib.scan.snyk_scanner.subprocess.run")
    def test_parses_findings_correctly(self, mock_run, mock_available):
        mock_available.return_value = True
        mock_run.return_value = MagicMock(
            returncode=0,
            stdout=json.dumps(
    ...
    @patch("lib.scan.snyk_scanner.is_snyk_scanner_available")
    @patch("lib.scan.snyk_scanner.subprocess.run")
    def test_handles_timeout(self, mock_run, mock_available):
        mock_available.return_value = True
        mock_run.side_effect = subprocess.TimeoutExpired(cmd="uvx", timeout=30)
        findings = run_snyk_scanner("/tmp/skill")
        assert findings == []
    ...
    @patch("lib.scan.snyk_scanner.is_snyk_scanner_available")
    @patch("lib.scan.snyk_scanner.subprocess.run")
    def test_handles_json_error(self, mock_run, mock_available):
        mock_available.return_value = True
        mock_run.return_value = MagicMock(
            returncode=0,
    ...
    @patch("lib.scan.snyk_scanner.is_snyk_scanner_available")
    @patch("lib.scan.snyk_scanner.subprocess.run")
    def test_handles_alternative_format(self, mock_run, mock_available):
    ...
    @patch("lib.scan.snyk_scanner.is_snyk_scanner_available")
    @patch("lib.scan.snyk_scanner.subprocess.run")
    def test_boosts_confidence_for_critical_threats(self, mock_run, mock_available):
