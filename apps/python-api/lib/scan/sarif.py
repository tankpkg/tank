...
def to_sarif(
    ...
    for finding in findings:
        ...
        if rule_id not in rules:
            rules[rule_id] = {
                "id": rule_id,
                "shortDescription": {"text": _format_rule_description(rule_id)},
            ...
            result["codeFlows"] = [
                {
                    ...
                    "threadFlows": [{"locations": [{"location": {"message": {"text": str(evidence)[:500]}}}]}],
                }
            ]
    ...
    if scan_duration_ms:
        sarif["runs"][0]["invocations"][0]["properties"] = {"duration_ms": scan_duration_ms}
