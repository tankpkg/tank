"""Stage 3: Prompt Injection Detection — Pattern Definitions

Compiled regex patterns for injection detection, hidden content detection,
base64 obfuscation checks, and heuristic suspicion scoring.
"""
...
DIRECT_OVERRIDE_PATTERNS = [
    (
        r"ignore\s+(all|any)?\s*(previous|prior|above|earlier)\s*(instructions?|prompts?|rules?|guidelines?)",
        "critical",
        1.0,
    ),
...
CONTEXT_MANIPULATION_PATTERNS = [
    (
        r"the\s+(following|real|actual|true)\s+(is|are)\s+(the|your)?\s*(real|actual|true)?\s*(system\s*)?(prompt|instructions?)",
        "high",
        0.9,
    ),
...
EXFILTRATION_PATTERNS = [
    ...
    (
        r"(output|print|display|show|reveal)\s+(the|your|all)\s*(system\s+prompt|instructions?|context|environment|secrets?)",
        "critical",
        1.0,
    ),
...
PRIVILEGE_ESCALATION_PATTERNS = [
    ...
    (
        r"(ignore|bypass|skip|disable)\s+(all\s+)?(safety|security|content)\s*(checks?|filters?|restrictions?)",
        "critical",
        1.0,
    ),
...
for _patterns in [
    ...
    for _pattern_str, _severity, _weight in _patterns:
        try:
            _compiled = re.compile(_pattern_str, re.IGNORECASE)
            ALL_PATTERNS.append((_compiled, _severity, _weight))
        except re.error:
...
def compute_suspicion_score(content: str, matched_patterns: list[tuple[str, float]]) -> float:
    ...
    imperative_count = len(re.findall(r"\b(do|must|should|need|have to|always|never)\b", content, re.IGNORECASE))
