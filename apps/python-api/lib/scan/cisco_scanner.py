...
def run_skill_scanner(skill_dir: str, use_behavioral: bool = True) -> list[Finding]:
    ...
    if not is_skill_scanner_available():
        logger.debug("skill-scanner not installed. Install with: pip install skill-scanner")
        return findings
    ...
    try:
        ...
        for finding_data in data.get("findings", []):
            ...
            findings.append(
                Finding(
                    stage="stage3",
                    severity=CISCO_TO_TANK_SEVERITY.get(finding_data.get("severity", "MEDIUM").upper(), "medium"),
