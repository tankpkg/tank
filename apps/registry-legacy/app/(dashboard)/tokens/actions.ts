...
function normalizeScopes(scopes: string[] | undefined): string[] {
  ...
  const normalized = Array.from(
    new Set(scopes.map((scope) => scope.trim()).filter((scope) => scope.length > 0 && allowedScopes.has(scope)))
  );
...
export async function createToken(input: { name: string; expiresInDays?: number; scopes?: string[] }) {
