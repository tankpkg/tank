...
const DEFAULT_STAGES: PipelineStage[] = [
  ...
  },
  {
    ...
    findingCount: 0,
    durationMs: 0,
    tools: ['Cisco Skill Scanner', 'Regex patterns']
  },
  {
    id: 'stage4',
...
function _getLineColor(status: string): string {
  switch (status) {
    case 'passed':
      return 'bg-green-500';
    case 'flagged':
...
export function ScanPipeline({ stages = DEFAULT_STAGES }: ScanPipelineProps) {
  ...
  const _hasFailed = displayStages.some((s) => s.status === 'failed');
  const _hasFlagged = displayStages.some((s) => s.status === 'flagged');
        ...
        {displayStages.map((stage, index) => (
          <div key={stage.id} className="relative">
            ...
            <div className="flex items-start gap-3 pb-4 relative z-10">
              {/* Status Icon */}
              <div
                className={`flex items-center justify-center w-6 h-6 rounded-full bg-background border-2 ${getStatusColor(
                  stage.status
                )} ${
                  stage.status === 'passed'
                    ? 'border-green-500'
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  ...
                  {stage.durationMs > 0 && (
                    <span className="text-xs text-muted-foreground">{(stage.durationMs / 1000).toFixed(1)}s</span>
                  )}
                </div>
                ...
                {stage.tools.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-1.5">
                    {stage.tools.map((tool) => (
                      <span key={tool} className="text-[10px] px-1.5 py-0.5 bg-muted rounded">
                        {tool}
                      </span>
                    ))}
                  </div>
