...
describe('scanCommand', () => {
  ...
  it('throws when not logged in', async () => {
    ...
    fs.writeFileSync(
      path.join(noAuthDir, 'config.json'),
      JSON.stringify({ registry: 'https://tankpkg.dev' }),
    );

    await expect(
      scanCommand({ directory: tmpDir, configDir: noAuthDir }),
  ...
  it('throws when packing fails', async () => {
    ...
    await expect(
      scanCommand({ directory: tmpDir, configDir }),
  ...
  it('displays scan results for a clean skill', async () => {
    ...
    mockFetch.mockResolvedValueOnce(
      new Response(JSON.stringify(makeScanResponse()), { status: 200 }),
    );
  ...
  it('displays findings grouped by severity', async () => {
    ...
    mockPack.mockResolvedValueOnce(mockPackResult);
    mockFetch.mockResolvedValueOnce(
      new Response(JSON.stringify(makeScanResponse({
        verdict: 'flagged',
        audit_score: 4.5,
        findings: [
          {
  ...
  it('throws on non-401 API error', async () => {
    ...
    mockPack.mockResolvedValueOnce(mockPackResult);
    mockFetch.mockResolvedValueOnce(
      new Response(JSON.stringify({ error: 'Skill too large' }), { status: 413 }),
    );

    await expect(
      scanCommand({ directory: tmpDir, configDir }),
    ).rejects.toThrow(/skill too large/i);
  });
  ...
  it('handles scan response without scan_id', async () => {
    ...
    mockPack.mockResolvedValueOnce(mockPackResult);
    mockFetch.mockResolvedValueOnce(
      new Response(JSON.stringify(makeScanResponse({ scan_id: null })), { status: 200 }),
    );
