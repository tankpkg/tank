...
export async function resolveDependencyTree(
  ...
  while (queue.size > 0) {
    ...
    const selectedVersion = findSatisfyingVersion(
      availableVersions,
      constraints,
    );
    ...
    if (previousVersion !== undefined) {
      ...
      for (const depName of prevDeps) {
        ...
        if (depConstraints) {
          const filtered = depConstraints.filter(
            (r) => r.source.from !== prevKey,
          );
    ...
    const sortedDepEntries = Object.entries(meta.dependencies).sort(
...
function findSatisfyingVersion(
  ...
  for (let i = 1; i < satisfyingSets.length; i++) {
    intersection = new Set(
      [...intersection].filter((v) => satisfyingSets[i].has(v)),
    );
  }
...
function buildGraph(
  ...
  const sortedEntries = [...selectedByName.entries()].sort(
    ([a], [b]) => a.localeCompare(b),
  );
  ...
  for (const [name, version] of sortedEntries) {
    ...
    if (!meta) {
      throw new Error(
        `Internal error: missing metadata for ${skillKey}`,
      );
    }
