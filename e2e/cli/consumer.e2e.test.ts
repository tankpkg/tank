...
describe('Consumer E2E — install and manage skills', () => {
  ...
  it('install downloads and extracts skill', async () => {
    ...
    if (!matchingKey) {
      throw new Error(`Expected lockfile entry for ${skill.name}`);
    }
  ...
  it('verify fails when skill directory is deleted', async () => {
    ...
    const output = result.stdout + result.stderr;
    expect(output).toMatch(/verification failed|integrity mismatch|file missing|tampered/i);
  });
  ...
  it('search works without auth (public endpoint)', async () => {
    const noAuthHome = fs.mkdtempSync(path.join(os.tmpdir(), 'tank-noauth-search-'));
