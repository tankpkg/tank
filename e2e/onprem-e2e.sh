...
post() { curl -s -X POST "$BASE$1" -H 'Content-Type: application/json' -d "$2" -b $COOKIE_JAR -c $COOKIE_JAR; }
...
echo "▸ Tokens: CRUD"
# Create
R=$(post /api/auth/create-api-key '{"name":"e2e-token","expiresIn":86400}')
# Better Auth's createApiKey returns the key object
TOKEN_ID=""
if echo "$R" | grep -q '"id"'; then
  ...
  # Try via our server function endpoint
  echo "  ⚠️  Direct auth API failed, testing via server function..."
fi

# List
R=$(get /api/auth/list-api-keys)
if echo "$R" | grep -q '\['; then
  assert_contains "List tokens returns array" '\[' "$R"
else
  echo "  ⚠️  list-api-keys returned: $(echo $R | head -c 80)"
fi

# Revoke (if we got an ID)
if [ -n "$TOKEN_ID" ]; then
  R=$(post /api/auth/delete-api-key "{\"keyId\":\"$TOKEN_ID\"}")
  assert_contains "Revoke token" '"success"' "$R"
else
...
post /api/auth/sign-up/email '{"email":"testuser@e2e.test","password":"testpass12345","name":"Test User"}' > /dev/null 2>&1
sleep 1
...
if [ -n "$TEST_USER_ID" ]; then
  # Suspend
  R=$(curl -s -X PATCH "$BASE/api/admin/users/$TEST_USER_ID/status" -H 'Content-Type: application/json' -b $COOKIE_JAR -d '{"status":"suspended","reason":"E2E test"}')
  ...
  # Reactivate
  R=$(curl -s -X PATCH "$BASE/api/admin/users/$TEST_USER_ID/status" -H 'Content-Type: application/json' -b $COOKIE_JAR -d '{"status":"active"}')
