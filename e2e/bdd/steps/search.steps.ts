...
import { Given, When, Then, runTank } from './fixtures';
...
Given('Alice has published a public skill {string}', async ({ e2eContext, publishedPublicSkill, bddState }, skillName: string) => {
...
Then('{word} should see {string} in the results', async ({ e2eContext, bddState }, actor: ActorName, skillName: string) => {
...
Then('{word} should not see {string} in the results', async ({ e2eContext, bddState }, actor: ActorName, skillName: string) => {
  if (!bddState.lastResult) {
    throw new Error('No CLI result available for search assertion.');
  }
  expect(bddState.lastActor).toBe(actor);
