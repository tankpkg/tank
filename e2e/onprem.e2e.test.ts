...
const TANK_CLI = join(process.cwd(), 'apps/cli/bin/tank.ts')
const NO_COLOR = 'NO_COLOR=1'

async function runTank(args: string[], options?: { env?: Record<string, string> }): Promise<{ stdout: string; stderr: string; exitCode: number }> {
