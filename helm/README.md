...
The scanner depends on postgres being healthy. If postgres takes longer than expected to initialize (e.g., on slow disk), increase the health check retries in `docker-compose.yml` or wait and restart:
...
```bash
helm upgrade tank helm/tank/ -f values-external-pg.yaml
```
...
```bash
helm dependency update helm/tank/
helm upgrade tank helm/tank/ --namespace tank
```
...
```bash
helm diff upgrade tank helm/tank/ --namespace tank
```
...
- `docker-compose.yml` and `.env.example` — configuration templates
